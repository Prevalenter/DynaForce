...
import utils.identification.SymPyBotics.sympybotics as spb

def gen_regr_matrices(rbt, q, dq, ddq, tau):


    loc = locals()
    # regressor_func_str = spb.robot_code_to_func( 'python', rbt.H_code, 'H', 'regressor_func', rbt.rbtdef)
    # exec(regressor_func_str)
    
    
    exec(spb.robot_code_to_func( 'python', rbt.H_code, 'H', 'regressor_func', rbt.rbtdef))
    regressor_func = loc['regressor_func']
...
def get_momentum_regr_matrices(rbt, q, dq, ddq, tau):
    """
    Compute the inertia matrix M(q) for each sample in q.
    Returns a numpy array of shape (num_samples, dof, dof)
    """
    loc = locals()
    exec(spb.robot_code_to_func('python', rbt.M_code, 'M', 'regressor_func_M', rbt.rbtdef))
    regressor_func_M = loc['regressor_func_M']

    num_samples = q.shape[0]
    dof = rbt.dof
    M_matrices = numpy.zeros((num_samples, dof, dof))

    # Try to get the dynamic parameters
    if hasattr(rbt, 'parms'):
        parms = rbt.parms
    elif hasattr(rbt, 'dynparms'):
        parms = rbt.dynparms
    elif hasattr(rbt, 'rbtdef') and hasattr(rbt.rbtdef, 'dynparms'):
        parms = rbt.rbtdef.dynparms()
    else:
        raise AttributeError('Could not find dynamic parameters in rbt object.')

    # breakpoint()

    for i in range(num_samples):
        M = regressor_func_M(q[i], parms)
        M = numpy.array(M).reshape((dof, dof))
        M_matrices[i] = M
