...
hand = gym.create_actor(env, cartpole_asset, initial_pose, 'gx11promax', 2, 1)
...
while not gym.query_viewer_has_closed(viewer):
    if idx < traj.shape[0]-1:
        # gym.set_actor_dof_position_targets(env, hand, traj[idx])
        idx += 1
    else:
        break
    # step the physics
    ...
    gym.set_actor_dof_position_targets(env, hand, list(traj[idx]))
    # gym.set_actor_dof_position_targets(env, hand, [0.2]*11)
    
    dof_state = gym.get_actor_dof_states(env, hand, gymapi.STATE_ALL)
    
    # breakpoint()    
    # cur_pos_i = gym.get_dof_position(env, hand)
    # cur_vel_i = gym.get_dof_velocity(env, hand)
