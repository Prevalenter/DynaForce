...
if __name__ == '__main__':
    ...
    for t in range(int(1e3)):
        ...
        recoder_pos_list.append(cur_pos.copy())
        recoder_vel_list.append(cur_vel.copy())
        recoder_acc_list.append(cur_acc.copy())
        recoder_torque_list.append(cur.copy())
        ...
        if t%100==99:
            print('save the data')
            np.save(f'{data_root_dir}/{exp_name}/pos_list.npy', np.array(recoder_pos_list))
            np.save(f'{data_root_dir}/{exp_name}/vel_list.npy', np.array(recoder_vel_list))
            np.save(f'{data_root_dir}/{exp_name}/acc_list.npy', np.array(recoder_acc_list))
            np.save(f'{data_root_dir}/{exp_name}/torque_list.npy', np.array(recoder_torque_list))
            np.save(f'{data_root_dir}/{exp_name}/force_pred_list.npy', np.array(recoder_pred_force_list))
